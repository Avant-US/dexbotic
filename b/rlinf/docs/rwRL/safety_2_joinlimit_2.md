...
> - 验证:64/64 Galaxea 单测在 Orin 真机上通过(11 老 safety + 16 新 L2 + 16 L3a + 6 schema + 7 camera mux + 8 hardware)。L2 单测三轮稳定 16/16。
...
夹爪在 RLinf 的 action layout 中通常是**单一标量**(M1: action[6];M2: action[6] 与 action[13];Behavior: 第 21/22 维)。它有两个 L2 约束,**实施版按"先 rate 再 pos"两步串行**(不是 plan 草稿那样的两个独立 if):
...
- **顺序敏感:rate 先于 pos**(实施期对 plan §7.6 的两个 if-else 写法做了串行化)。原因:`rate_cap` 改的是 `new_a`,`pos_cap` 用 `new_a * scale_g` 计算 `next_pct`,如果先做 pos_cap 再做 rate_cap,有可能"先把目标 pct 锁回 hi,再 rate-limit 到一个**不到 hi** 的 next_pct"。串行 rate→pos 才能保证最终 `safe_action[gripper]` 同时尊重两个约束。
- **`scale_g = max(action_scale[2], 1e-9)`**:防止用户把 gripper scale 设为 0 时除零;同时这也让 plan 草稿的 `max(scale_g, 1e-6)` 改成 `1e-9`(更保守)。
- **L1 与 gripper scale 的相互作用是 L2 测试的隐藏陷阱**(实施期发现):RLinf 默认 `action_scale[2] = 1.0`,在测试中如果用 `action[6] = 50.0` 想测"50%/step delta",L1 会把 `50.0` clip 到 `1.0`,L2 gripper 看到的就只是 `1×1=1%/step` 的小变化,根本触发不了 rate/pos cap。**测试时必须用 `action_scale[2]=50` 让 `a ∈ [-1,1]` 对应 ±50%/step 才能有效校验**(参见单测 `test_l2_gripper_position_clamp_caps_to_max` / `test_l2_gripper_rate_limit_caps_step` 的 schema 设置)。详见 [附录 C.4](#c4-l1-clip-与-gripper-scale-的相互作用)。
...
(`get_arm_qpos / get_arm_qvel / get_gripper_pos` 已经在文件中存在。实施版**比 plan 草稿多了一个 `get_chassis_qpos`** —— 加上是为了将来扩展底盘 odom-based safety 时不必再回头改 dataclass。)
...
| [`r1_pro_robot_state.py`](../../../rlinf/envs/realworld/galaxear/r1_pro_robot_state.py) | 160-180 | helper:`get_torso_qpos/qvel`、`get_chassis_qpos/qvel`、`get_gripper_vel` | **已加**(§8.1) |
...
### C.4 L1 clip 与 gripper scale 的相互作用
...
assert "L2:right_gripper" in info.reason   # FAILED
```

测试**失败**:`info.reason` 里没有 L2 gripper tag。

#### 根因

`validate()` 的 L1 段在跑 L2 之前先把 action clip 到 `[-1, 1]`:
...
`a[6] = 50.0 → 1.0`。然后 L2 gripper 看到的是 `a_g = 1.0, scale = 1.0 → delta = 1.0 %/step`,远小于 `max_step_pct = 10`,也不会让 `next_pct = 50 + 1 = 51` 越界 `[0, 100]`。所以 L2 gripper 一切正常,**单测意图错了**。

#### 修复

测试用更合理的 gripper scale:

```python
schema = ActionSchema(action_scale=[0.05, 0.10, 50.0])  # gripper scale = 50
a = np.zeros(7); a[6] = 1.0   # 在 [-1,1] 内,L1 不 clip
# delta = 1.0 * 50.0 = 50.0 %/step → 触发 rate_cap
```

#### 给 maintainer 的注意点

- 生产代码中 `action_scale[2]` 的语义是"夹爪 1 normalised unit 等于多少 %/step"。
- RLinf 默认 1.0 表示"a=1 → +1%/step",这是个相对慢的设定,在 50 步可达全开/全关。**`max_gripper_step_pct = 30` 在这个默认 scale 下永远不触发**。
- 实际真机 bring-up 时建议把 `action_scale[2]` 设为 30~50,让 1 步内能完成抓取语义,然后让 `max_gripper_step_pct` 做实际的 rate-limit。
- 单测和现场调参都要意识到 L1 → L2 这个 clip 链条:**a 必须先在 [-1, 1] 内,scale 才决定单步真实物理量**。
...
> 写到这里,**L2 的"位置 + 速度"约束方案就完整了**——文档与代码、设计与实施、附录 C 的 5 个 corner case 与单测的 16 个用例,全部对齐。下一步是创建 PR、在 [`test_galaxea_r1_pro_controller.md`](test_galaxea_r1_pro_controller.md) §A.3.3 的 REPL 流程里验证 §11.2 的现场表现。完整的实施可以参照 [`r1pro5op47_imp1.md`](r1pro5op47_imp1.md) 的滚动实施模式。
