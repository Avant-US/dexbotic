...
## Merge Attention是如何实现的

DM0 的 merged attention 可以先用一句话理解：

**它不是“VLM 先看完图文，再把结果丢给 Action Head”，而是让 VLM token 和 action token 在每一层 Transformer 的 attention 里坐到同一张桌子上开会；但开会后，各自还是回到自己的专家网络里做后处理。**

### 先看两个专家

本地代码里的 DM0 有两套 Transformer：

- **VLM / LLM 分支**：负责图像 + 文本理解，也就是 prefix。
- **Action Expert 分支**：另一个 Qwen3 模型，但去掉了 `embed_tokens`，不再吃文本 token，而是吃连续动作 embedding，也就是 suffix。

```77:90:dexbotic/model/dm0/dm0_arch.py
...
通俗讲，VLM 是“看图读指令的人”，Action Expert 是“会写控制轨迹的人”。普通做法可能是前者写一份总结，后者读总结。DM0 更激进：**每一层都让两者直接互相注意到对方的 token**。

### Prefix 和 Suffix

在代码里，DM0 把输入拆成两段：

- **prefix**：多视角图像 token + 语言 token。
- **suffix**：带噪声的 action chunk + flow time embedding。

prefix 的构造：

```307:353:dexbotic/model/dm0/dm0_arch.py
...
```355:404:dexbotic/model/dm0/dm0_arch.py
...
也就是说，Action Expert 不是直接预测一个动作，而是拿到：

- 当前噪声动作 `x_t`
- 当前 flow 时间 `t`
- 图文观测条件 prefix

然后学习“这团噪声应该往哪个方向流动，才能变成真实动作”。

### Merged Attention 真正做了什么

核心在 `_compute_merged_layer()`。

每一层 Transformer 中，代码分别从 VLM 层和 Action Expert 层里算出自己的 Q/K/V：

```160:187:dexbotic/model/dm0/dm0_arch.py
...
然后把两个专家的 Q/K/V 在序列维度上拼起来：

```197:199:dexbotic/model/dm0/dm0_arch.py
        query_states = torch.cat(query_list, dim=2)
        key_states = torch.cat(key_list, dim=2)
        value_states = torch.cat(value_list, dim=2)
```

这一步就是 merged attention 的本质。

可以想象成：

```text
VLM tokens:          [image tokens, language tokens]
Action Expert tokens:[noisy action 1, noisy action 2, ..., noisy action 50]

普通方案:
VLM 自己 attention
Action Expert 自己 attention 或 cross-attention VLM

DM0:
Q = concat(Q_vlm, Q_action)
K = concat(K_vlm, K_action)
V = concat(V_vlm, V_action)
然后一次 attention 算完
```

算完 attention 后，再把输出切回两段：

```242:266:dexbotic/model/dm0/dm0_arch.py
...
关键点：**attention 是合并的，但每个分支的 `o_proj`、LayerNorm、MLP 仍然用自己的参数。**

所以它不是简单把两个模型糊成一个模型，也不是共享全部参数。更准确地说：

> 两个专家各自生成 Q/K/V，在 attention 空间里临时合流；attention 结束后，再回到各自专家的 FFN/MLP 路径中继续处理。

### Attention Mask 如何控制信息流

如果所有 token 都互相看，会有信息泄漏。DM0 用 `make_attn_mask_2d()` 控制谁能看谁：

```37:40:dexbotic/model/dm0/dm0_utils.py
    cumsum = torch.cumsum(attn_mask, dim=1)
    attn_mask_2d = cumsum[:, None, :] <= cumsum[:, :, None]
...
训练时，代码把 prefix 和 suffix 的 mask 拼起来：

```461:470:dexbotic/model/dm0/dm0_arch.py
...
- prefix 不能看未来的 suffix action，避免“答案泄漏”。
- suffix 可以看 prefix，因为动作生成必须依赖图像和指令。
- suffix 内部的动作 chunk 可以一起建模，所以适合并行预测一整段未来动作。

这也是 DM0 和纯自回归 action token 方法的区别：**DM0 的连续动作分支不是一个 token 一个 token 慢慢生成，而是通过 Flow Matching 并行生成一个 chunk。**

### 训练时怎么用

训练时先构造带噪声动作：

```428:443:dexbotic/model/dm0/dm0_arch.py
        noise = torch.normal(
...
然后 prefix + suffix 一起走 merged attention：

```480:493:dexbotic/model/dm0/dm0_arch.py
...
最后只拿 `suffix_out` 预测 velocity field：

```495:500:dexbotic/model/dm0/dm0_arch.py
...
所以在当前开源代码里，merged attention 的主要服务对象是 **Flow Matching 连续动作生成**。

### 推理时怎么用

推理时先只跑 prefix，得到 KV cache：

```540:568:dexbotic/model/dm0/dm0_arch.py
...
然后每一步去噪时，只送 suffix action tokens，但它们通过 KV cache 继续看 prefix：

```631:638:dexbotic/model/dm0/dm0_arch.py
        (_, suffix_out), _ = self._merged_attention_forward(
            module_list=module_list,
...
1. 先看图和指令，形成“任务上下文缓存”。
2. 从随机噪声动作开始。
3. 每一步让 Action Expert 根据图文上下文，把动作往正确方向修一点。
4. 多步之后得到 50 步连续动作。

### 和 Cross-Attention 的区别

很多 VLA 会这么做：

```text
VLM encoder 输出 hidden states
Action head 通过 cross-attention 读取这些 hidden states
```
...
每一层:
  VLM layer 产生 Q/K/V
  Action Expert layer 产生 Q/K/V
  拼成一个大 Q/K/V
  一次 self-attention
  再切回 VLM 和 Action Expert
```

这比普通 cross-attention 更“深度耦合”。Action Expert 不是只在某几层读一次 VLM 结果，而是在每层 attention 中都能和图文 token 对齐。

但它又不是完全混成一个模型，因为：

- VLM 有自己的 LayerNorm、QKV、MLP。
- Action Expert 有自己的 LayerNorm、QKV、MLP。
- 只是 attention 计算时合并。
- 输出后仍然分开更新各自 hidden states。

### 一个形象比喻

可以把 DM0 想成两位专家：

- 视觉语言专家：看现场，理解“把 moka pot 放到炉子上”。
- 动作专家：负责生成机械臂 50 步动作。

普通 cross-attention 像是视觉语言专家写一份报告，动作专家读报告。

DM0 的 merged attention 像是两位专家每一轮思考都坐在同一个会议室：

- 视觉语言专家说：“目标物体在这里，任务是放到炉子上。”
- 动作专家说：“我当前这条动作轨迹有点偏，应该往这个方向修。”
- 两者在 attention 里直接互相参考。
- 会后各自回办公室，用自己的专业网络继续处理。

### 需要注意的代码现实

`b/d/dm0/` 的分析文档里多次指出：论文讲了 `L_AR + L_FM`、离散 action token、Embodied Spatial Scaffolding 等设计；但**当前本地开源代码里的 DM0 主要实现了 L_FM 分支**。

也就是说：

- merged attention 已实现。
- Flow Matching 连续动作生成已实现。
- `prefix_out` 当前训练里基本被丢弃。
- `labels` 参数存在但没有被用于 LM loss。
- 当前 `loss = action_loss`，没有真正加上论文里的 `L_AR`。

所以，本地代码版 DM0 的 merged attention 可以理解为：

> 用 merged attention 把图文理解 token 和连续动作去噪 token 深度耦合，让 Action Expert 在每一层都能条件化于 VLM 上下文，然后用 Flow Matching 生成一段连续动作。论文里的自回归离散动作监督在当前开源实现中还没有完整接上。